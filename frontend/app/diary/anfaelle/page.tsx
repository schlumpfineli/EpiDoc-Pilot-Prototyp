...
// Selbst eingetragene Auffälligkeiten/Auslöser liegen im Backend, lokal nur als Offline-Cache
const CUSTOM_OPTIONS_CACHE_KEY = "diary-custom-options";

const EMPTY_CUSTOM_OPTIONS: SeizureCustomOptions = { after_effects: [], triggers: [] };

function sanitizeCustomOptions(value: unknown): SeizureCustomOption[] {
  if (!Array.isArray(value)) return [];
  return value.filter(
    (entry): entry is SeizureCustomOption =>
      typeof entry === "object" &&
      entry !== null &&
      typeof (entry as SeizureCustomOption).id === "number" &&
      typeof (entry as SeizureCustomOption).label === "string"
  );
}

function loadCachedCustomOptions(): SeizureCustomOptions {
  if (typeof window === "undefined") return EMPTY_CUSTOM_OPTIONS;
  try {
    const parsed = JSON.parse(localStorage.getItem(CUSTOM_OPTIONS_CACHE_KEY) || "null");
    if (typeof parsed !== "object" || parsed === null) return EMPTY_CUSTOM_OPTIONS;
    return {
      after_effects: sanitizeCustomOptions((parsed as SeizureCustomOptions).after_effects),
      triggers: sanitizeCustomOptions((parsed as SeizureCustomOptions).triggers),
    };
  } catch {
...
function cacheCustomOptions(options: SeizureCustomOptions) {
  try {
    localStorage.setItem(CUSTOM_OPTIONS_CACHE_KEY, JSON.stringify(options));
  } catch {
    // Speicher nicht verfügbar (z. B. privater Modus) – Liste kommt beim nächsten Laden vom Server
  }
}

/** Alte lokale Listen aus der Zeit vor der Backend-Speicherung */
function loadLegacyCustomLabels(key: string): string[] {
...
/** Freitextfeld in einzelne Einträge zerlegen – Komma trennt mehrere Angaben */
function splitCustomEntries(raw: string): string[] {
  return raw
    .split(",")
    .map((value) => value.trim())
    .filter((value) => value !== "" && value.length <= 255);
}

/** Nur noch unbekannte Angaben zurückgeben – Duplikate (auch bei anderer Schreibweise) überspringen */
function newCustomLabels(
  raw: string,
  existing: SeizureCustomOption[],
  builtIn: readonly string[]
): string[] {
  const known = new Set(
    [...existing.map((option) => option.label), ...builtIn].map((label) => label.toLowerCase())
  );
  const fresh: string[] = [];

  for (const label of splitCustomEntries(raw)) {
...
export default function DiaryPage() {
  ...
  // Eigene Auffälligkeiten/Auslöser, die zur Auswahlliste hinzugekommen sind
  const [customOptions, setCustomOptions] = useState<SeizureCustomOptions>(EMPTY_CUSTOM_OPTIONS);
  ...
  useEffect(() => {
    // Cache zuerst zeigen, damit die Liste offline und beim Laden verfügbar bleibt
    setCustomOptions(loadCachedCustomOptions());

    let cancelled = false;

    const migrateLegacyOptions = async (current: SeizureCustomOptions): Promise<SeizureCustomOptions> => {
      let next = current;
      const legacy: Array<{ kind: SeizureCustomOptionKind; key: string; builtIn: readonly string[] }> = [
        { kind: "after_effect", key: "diary-custom-after-effects", builtIn: afterEffectsOptions },
        { kind: "trigger", key: "diary-custom-triggers", builtIn: triggerOptions },
      ];

      for (const item of legacy) {
        const labels = newCustomLabels(
          loadLegacyCustomLabels(item.key).join(","),
          item.kind === "after_effect" ? next.after_effects : next.triggers,
          item.builtIn
        );
        if (labels.length > 0) {
          const response = await seizureOptionApi.add({ kind: item.kind, labels });
          next = response.data;
        }
  ...
  // Eigene Einträge stehen in der Auswahlliste immer oben
  const customAfterEffectLabels = useMemo(
    () => customOptions.after_effects.map((option) => option.label),
    [customOptions.after_effects]
  );
  const customTriggerLabels = useMemo(
    () => customOptions.triggers.map((option) => option.label),
    [customOptions.triggers]
  );
  const afterEffectsChoices = useMemo(
    () => [...customAfterEffectLabels, ...afterEffectsOptions],
    [customAfterEffectLabels]
  );
  const triggerChoices = useMemo(
    () => [...customTriggerLabels, ...triggerOptions],
    [customTriggerLabels]
  );

  const applyCustomOptions = (options: SeizureCustomOptions) => {
    setCustomOptions(options);
    cacheCustomOptions(options);
  };

  /** Neu eingetippte Angaben im Backend hinterlegen, damit sie künftig oben in der Auswahl stehen */
  const persistCustomOptions = async () => {
    const additions: Array<{ kind: SeizureCustomOptionKind; labels: string[] }> = [
      {
        kind: "after_effect",
      ...
      {
        kind: "trigger",
        labels: newCustomLabels(formData.customTriggers, customOptions.triggers, triggerOptions),
      },
    ];

    for (const addition of additions) {
      if (addition.labels.length === 0) continue;
      try {
        const response = await seizureOptionApi.add(addition);
        applyCustomOptions(response.data);
      } catch {
        // Der Anfall selbst wurde gespeichert – die Angabe fehlt dann nur in der Auswahlliste
      }
    }
  };
  ...
  const handleRemoveCustomOption = async (kind: SeizureCustomOptionKind, label: string) => {
    ...
    try {
      const response = await seizureOptionApi.remove(option.id);
      applyCustomOptions(response.data);
    } catch (error: any) {
      toastService.show(
        error?.message || "Eintrag konnte nicht aus der Liste entfernt werden",
        "error"
      );
      return;
    }
    ...
    setFormData((prev) =>
      kind === "after_effect"
        ? { ...prev, afterEffects: prev.afterEffects.filter((value) => value !== label) }
        : { ...prev, triggers: prev.triggers.filter((value) => value !== label) }
    );
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Eigene Angaben serverseitig merken, damit sie auf allen Geräten oben in der Auswahl stehen
